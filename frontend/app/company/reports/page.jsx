...
export default function Reports() {
  ...
  const focusedCase = searchParams.get('case') || '';
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await listBGVCases({ page: 1, limit: 200, status: focusedCase ? undefined : 'COMPLETED', search: focusedCase || undefined, clientId: clientId || undefined, completedFrom: focusedCase ? undefined : completedFrom || undefined, completedTo: focusedCase ? undefined : completedTo || undefined }, accessToken);
      setReports(result.items); setSelected([]);
    } catch (err) { setError(err instanceof ApiError ? err.message : 'Could not load reports.'); }
    finally { setLoading(false); }
  }, [accessToken, clientId, completedFrom, completedTo, focusedCase]);
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <div><p className="text-[11px] uppercase tracking-[0.14em] text-[var(--primary)]">Completed verifications</p><h1 className="text-[26px] font-semibold">Reports</h1><p className="mt-1 text-[13px] text-[var(--muted)]">Filter completed cases, select reports for one client, and send them by email.</p>{focusedCase && <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-[#3FDCC0]/40 bg-[#3FDCC0]/10 px-3 py-1.5 text-[12px] font-semibold text-[#147A68]"><CheckSquare size={14} />Case {focusedCase} added to reports</div>}</div>
