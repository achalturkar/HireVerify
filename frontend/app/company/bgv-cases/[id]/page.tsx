...
function CaseOverview({ caseData, token, onSaved, readOnly = false }: { caseData: BGVCase; token: string | null; onSaved: (value: BGVCase) => void; readOnly?: boolean }) {
  ...
  const save = async () => { if (readOnly) return; setSaving(true); setError(null); try { const updatedCandidate = caseData.candidate ? await updateCandidate(caseData.candidate.id, { dateOfBirth: form.dateOfBirth || null }, token) : null; const updated = await updateBGVCaseMeta(caseData.id, form, token); if (updatedCandidate) onSaved({ ...updated, candidate: { ...updated.candidate, ...updatedCandidate } }); else onSaved(updated); } catch (err) { setError(err instanceof ApiError ? err.message : 'Could not save case.'); } finally { setSaving(false); } };
  return <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 className="mb-4 text-[15px] font-semibold">Candidate information</h2>{error && <p className="mb-3 text-[13px] text-[#FF6B6B]">{error}</p>}{readOnly && <p className="mb-4 rounded-md border border-[#F2AE55]/30 bg-[#F2AE55]/10 px-3 py-2 text-[12px] text-[#8A5A12]">Candidate information is view-only while the case is COMPLETED. Change the case status before making edits.</p>}<div className="grid gap-4 sm:grid-cols-2"><Field label="Candidate name" value={`${caseData.candidate?.firstName || ''} ${caseData.candidate?.lastName || ''}`} disabled /><Field label="Gender" value={caseData.candidate?.gender || ''} disabled /><Field label="Date of birth" type="date" value={form.dateOfBirth} onChange={(value) => update('dateOfBirth', value)} disabled={readOnly} /><Field label="Email" value={caseData.candidate?.email || ''} disabled /><Field label="Phone" value={caseData.candidate?.phone || ''} disabled /><Field label="BVPL reference" value={caseData.caseNumber} disabled /><Field label="Client reference" value={form.clientReference} onChange={(value) => update('clientReference', value)} disabled={readOnly} /><Field label="Entity / package" value={form.packageName} onChange={(value) => update('packageName', value)} disabled={readOnly} /><Field label="Date initiated" type="date" value={form.initiatedAt} onChange={(value) => update('initiatedAt', value)} disabled={readOnly} /><Field label="Date completed" type="date" value={form.completedAt} onChange={(value) => update('completedAt', value)} disabled={readOnly} /></div><button onClick={save} disabled={saving || readOnly} className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[var(--primary)] px-4 py-2.5 text-[13px] font-semibold text-[var(--primary-foreground)] disabled:opacity-50"><Save size={14} />{saving ? 'Saving...' : 'Save case'}</button></section>;
}
