...
import { ArrowLeft, FileDown, ListChecks, Lock, Save, Unlock } from 'lucide-react';
...
export default function BGVCaseWorkspacePage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    {activeTab === 'overview' ? <CaseOverview caseData={caseData} token={accessToken} onSaved={setCaseData} /> : selectedCheck ? <VerificationEditor check={selectedCheck} token={accessToken} onSaved={saveCheck} /> : <p className="text-[13px] text-[var(--muted)]">This verification is not included in this case.</p>}
  </div>;
}

const verifierOptions = ['Achal Turkar', 'Niraj Chaudhary', 'Prasanna Yadav', 'Suhas Birewar', 'Ruchita N Pawar', 'Neha T', 'Adv. Rishabh Vakharia', 'Nandini'];

function Field({ label, value, onChange, disabled = false, type = 'text' }: { label: string; value: string; onChange?: (value: string) => void; disabled?: boolean; type?: string }) { const [manual, setManual] = useState(Boolean(value && !verifierOptions.includes(value))); useEffect(() => { if (value && !verifierOptions.includes(value)) setManual(true); }, [value]); return <label><span className="mb-1.5 block text-[12.5px] font-medium">{label}</span>{label === 'Verified by' ? <><select value={manual ? '__manual__' : value} disabled={disabled} onChange={(event) => { const selected = event.target.value; setManual(selected === '__manual__'); onChange?.(selected === '__manual__' ? '' : selected); }} className={inputClass}><option value="">Select verifier</option>{verifierOptions.map((name) => <option key={name} value={name}>{name}</option>)}<option value="__manual__">Other / enter manually</option></select>{manual && <input type="text" value={value} placeholder="Enter verifier name" disabled={disabled} onChange={(event) => onChange?.(event.target.value)} className={`${inputClass} mt-2`} />}</> : <input type={type} value={value} disabled={disabled} onChange={(event) => onChange?.(event.target.value)} className={inputClass} />}</label>; }
...
function CaseOverview({ caseData, token, onSaved }: { caseData: BGVCase; token: string | null; onSaved: (value: BGVCase) => void }) {
  const [form, setForm] = useState({ packageName: caseData.packageName || '', clientReference: caseData.clientReference || '', remarks: caseData.remarks || '', initiatedAt: caseData.initiatedAt?.slice(0, 10) || '', completedAt: caseData.completedAt?.slice(0, 10) || '', status: caseData.status });
  const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null);
  const update = (key: keyof typeof form, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const save = async () => { setSaving(true); setError(null); try { let updated = await updateBGVCaseMeta(caseData.id, form, token); if (form.status !== caseData.status) updated = await transitionBGVCase(caseData.id, form.status, form.remarks, token); onSaved(updated); } catch (err) { setError(err instanceof ApiError ? err.message : 'Could not save case.'); } finally { setSaving(false); } };
  return <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 className="mb-4 text-[15px] font-semibold">Candidate information</h2>{error && <p className="mb-3 text-[13px] text-[#FF6B6B]">{error}</p>}<div className="grid gap-4 sm:grid-cols-2"><Field label="Candidate name" value={`${caseData.candidate?.firstName || ''} ${caseData.candidate?.lastName || ''}`} disabled /><Field label="Gender" value={caseData.candidate?.gender || ''} disabled /><Field label="Date of birth" value={caseData.candidate?.dateOfBirth?.slice(0, 10) || ''} disabled /><Field label="Email" value={caseData.candidate?.email || ''} disabled /><Field label="Phone" value={caseData.candidate?.phone || ''} disabled /><Field label="BVPL reference" value={caseData.caseNumber} disabled /><Field label="Client reference" value={form.clientReference} onChange={(value) => update('clientReference', value)} /><Field label="Entity / package" value={form.packageName} onChange={(value) => update('packageName', value)} /><Field label="Date initiated" type="date" value={form.initiatedAt} onChange={(value) => update('initiatedAt', value)} /><Field label="Date completed" type="date" value={form.completedAt} onChange={(value) => update('completedAt', value)} /><label><span className="mb-1.5 block text-[12.5px] font-medium">Case status</span><select value={form.status} onChange={(event) => update('status', event.target.value)} className={inputClass}>{['DRAFT', 'INITIATED', 'CONSENT_PENDING', 'IN_PROGRESS', 'UNDER_REVIEW', 'COMPLETED', 'ON_HOLD', 'CANCELLED'].map((value) => <option key={value}>{value}</option>)}</select></label><TextArea label="Current address" value={caseData.candidate?.currentAddress || ''} disabled rows={2} /><TextArea label="Permanent address" value={caseData.candidate?.permanentAddress || ''} disabled rows={2} /><div className="sm:col-span-2"><TextArea label="Remarks" value={form.remarks} onChange={(value) => update('remarks', value)} /></div></div><button onClick={save} disabled={saving} className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[var(--primary)] px-4 py-2.5 text-[13px] font-semibold text-[var(--primary-foreground)] disabled:opacity-50"><Save size={14} />{saving ? 'Saving...' : 'Save case'}</button></section>;
}

const initialData = (check: VerificationCheck) => { const value = check.resultData && typeof check.resultData === 'object' ? check.resultData as Record<string, unknown> : {}; return { ...value, verifierName: String(value.verifierName || value.verifiedBy || ''), remarks: String(value.remarks || check.remarks || ''), outcome: String(value.outcome || value.status || (check.result === 'VERIFIED' ? 'CLEAR' : '')) }; };
...
function VerificationEditor({ check, token, onSaved }: { check: VerificationCheck; token: string | null; onSaved: (value: VerificationCheck) => void }) {
  ...
  useEffect(() => { setData(initialData(check)); setStatus(check.status); setError(null); }, [check.id]);
