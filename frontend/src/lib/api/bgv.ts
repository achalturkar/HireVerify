...
export const resolveFileUrl = (fileUrl: string) => /^(https?:|blob:|data:)/i.test(fileUrl) ? fileUrl : `${API_ORIGIN}${fileUrl}`;
...
export async function uploadVerificationDocument(id: string, candidateId: string, file: File, documentType: string, token: string | null): Promise<VerificationCheck> { const form = new FormData(); form.append('file', file); form.append('candidateId', candidateId); form.append('documentType', documentType); const response = await fetch(`${API_BASE}/bgv/verifications/${id}/documents`, { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : {}, body: form }); const body = await response.json().catch(() => null); if (!response.ok || body?.success === false) throw new ApiError(typeof body?.message === 'string' ? body.message : body?.message?.message || `Request failed (${response.status})`, response.status); return body.data?.data ?? body.data; }
...
export async function emailBGVReports(caseIds: string[], token: string | null): Promise<void> { await request<{ message: string }>('/bgv/cases/reports/email', token, { method: 'POST', body: JSON.stringify({ caseIds }) }); }
